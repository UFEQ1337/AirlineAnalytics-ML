"""
🇪🇺 MODELE MACHINE LEARNING - DANE EUROPEJSKIE
==============================================

Specjalistyczne modele dostosowane do danych europejskich:
- Uwzględnia specyfikę europejską (strajki, kraje, krótsze dystanse)
- Features geograficzne (kraje, regiony)
- Sezonowość europejska
- Przewoźników europejskich (LOT, Ryanair, Wizz Air, etc.)
...
class EuropeanDelayClassifier:
    """
    🇪🇺 Klasyfikator opóźnień - wersja europejska
    Przewiduje czy lot będzie opóźniony (>15 min) na podstawie danych europejskich
    """
    
    def __init__(self, model_type: str = 'xgboost'):
        """
        Inicjalizacja europejskiego klasyfikatora
        ...
        self.problematic_airports = ['LHR', 'CDG', 'FRA', 'AMS']  # Duże huby z problemami
        self.polish_airports = ['WAW', 'KRK', 'GDN', 'WRO', 'KTW', 'POZ', 'RZE', 'LUZ']
        self.low_cost_carriers = ['Ryanair', 'Wizz Air', 'easyJet', 'Norwegian']
        
        # Inicjalizacja modelu z poprawkami dla class imbalance
        if model_type == 'logistic':
            ...
            # Oblicz scale_pos_weight dla XGBoost (stosunek negative do positive)
            self.model = xgb.XGBClassifier(random_state=42, eval_metric='logloss', scale_pos_weight=4)  # ~80/20 ratio
        else:
    ...
    def prepare_european_features(self, df: pd.DataFrame, prediction_mode: bool = False) -> Tuple[pd.DataFrame, Optional[pd.Series]]:
        """
        🇪🇺 Feature Engineering - europejska wersja z dodatkowymi cechami
        ...
        """
        print("🇪🇺 EUROPEAN FEATURE ENGINEERING - KLASYFIKACJA")
        print("="*60)
        ...
        df_features['is_summer_holiday'] = df_features['month'].isin([7, 8]).astype(int)  # Europejskie wakacje
        df_features['is_winter_season'] = df_features['month'].isin([12, 1, 2]).astype(int)  # Problemy zimowe
        df_features['is_spring_autumn'] = df_features['month'].isin([3, 4, 9, 10]).astype(int)  # Najlepsze warunki
        
        # European rush hours (różne niż USA)
        ...
        # 4. DISTANCE FEATURES (kilometry europejskie)
        if 'distance_km' in df_features.columns:
            df_features['distance_normalized'] = df_features['distance_km'] / df_features['distance_km'].max()
            
            # European distance categories (krótsze niż USA)
            ...
            # Short European flights indicator
            df_features['short_eu_flight'] = (df_features['distance_km'] < 800).astype(int)
            df_features['medium_eu_flight'] = ((df_features['distance_km'] >= 800) & 
                                             (df_features['distance_km'] < 1500)).astype(int)
        ...
        # 7. SEASONAL EUROPEAN PATTERNS
        print("🌦️ Europejska sezonowość...")
        # European holiday periods
        df_features['christmas_period'] = df_features['month'].isin([12]).astype(int)
        df_features['easter_period'] = df_features['month'].isin([3, 4]).astype(int)  # Rough approximation
        df_features['summer_peak'] = ((df_features['month'].isin([7, 8])) & 
                                     (df_features['day_of_week'].isin([4, 5, 6]))).astype(int)
        
        # 8. ONE-HOT ENCODING dla kategorii
        print("🏷️ One-hot encoding...")
        categorical_cols = []
        
        # Top airlines (ograniczone do 10 + Other)
        if 'airline' in df_features.columns:
            top_airlines = df_features['airline'].value_counts().head(10).index
            df_features['airline_cat'] = df_features['airline'].apply(
                lambda x: x if x in top_airlines else 'Other'
            )
            airline_dummies = pd.get_dummies(df_features['airline_cat'], prefix='airline', drop_first=True)
            df_features = pd.concat([df_features, airline_dummies], axis=1)
        
        # Top routes (only most popular)
        if 'origin' in df_features.columns and 'destination' in df_features.columns:
            df_features['route'] = df_features['origin'] + '_' + df_features['destination']
            top_routes = df_features['route'].value_counts().head(15).index
            df_features['route_cat'] = df_features['route'].apply(
                lambda x: x if x in top_routes else 'Other'
            )
            route_dummies = pd.get_dummies(df_features['route_cat'], prefix='route', drop_first=True)
            df_features = pd.concat([df_features, route_dummies], axis=1)
        
        # Regions
        if 'region_origin' in df_features.columns:
            region_orig_dummies = pd.get_dummies(df_features['region_origin'], prefix='region_orig', drop_first=True)
            region_dest_dummies = pd.get_dummies(df_features['region_destination'], prefix='region_dest', drop_first=True)
            df_features = pd.concat([df_features, region_orig_dummies, region_dest_dummies], axis=1)
        
        # Distance categories
        if 'distance_category' in df_features.columns:
            distance_dummies = pd.get_dummies(df_features['distance_category'], prefix='dist', drop_first=True)
            df_features = pd.concat([df_features, distance_dummies], axis=1)
        
        # 9. SELECT FINAL FEATURES
        print("🔍 Selekcja finalnych cech...")
        
        # Exclude non-numeric and original categorical columns (oraz DATA LEAKAGE!)
        exclude_cols = [
            ...
            'aircraft_type', 'country_origin', 'country_destination', 'origin_city', 'destination_city',
            'region_origin', 'region_destination', 'distance_category', 'airline_cat', 'route_cat',
            'is_delayed',  # KRYTYCZNE: to jest TARGET VARIABLE - MUSI BYĆ WYKLUCZONE!
            # Usuwamy też wszystkie delay-based features aby uniknąć data leakage
            'weather_delay', 'strike_delay', 'air_traffic_delay', 'mechanical_delay', 'airport_issues_delay'
        ]
        ...
        print(f"✅ European feature engineering zakończony!")
        print(f"📊 Utworzono {len(self.feature_names)} cech europejskich")
        ...
        print(f"✈️ Cechy przewoźników: {[f for f in self.feature_names if any(x in f.lower() for x in ['lot', 'low_cost', 'lufthansa'])]}")
    ...
    def train(self, X: pd.DataFrame, y: pd.Series, test_size: float = 0.2, hyperparameter_tuning: bool = True):
        """
        🎯 Trenowanie europejskiego modelu klasyfikacji
        """
        print(f"🇪🇺 TRENOWANIE EUROPEJSKIEGO MODELU: {self.model_type.upper()}")
        print("="*60)
        ...
        if hyperparameter_tuning:
            ...
            self._tune_european_hyperparameters(X_train_scaled, y_train)
    ...
    def _tune_european_hyperparameters(self, X_train: np.ndarray, y_train: np.ndarray):
        """Optymalizacja hiperparametrów dla modeli europejskich"""
        ...
        grid_search = GridSearchCV(
            self.model, param_grid, cv=cv, scoring='roc_auc', n_jobs=-1, verbose=0
        )
    ...
    def predict_binary(self, X: Union[pd.DataFrame, np.ndarray]) -> np.ndarray:
        """Przewidywanie binarnego wyniku (opóźniony/punktualny)"""
        X_scaled = self.scaler.transform(X.values if hasattr(X, 'values') else X)
    ...
    def plot_european_feature_importance(self, top_n: int = 20):
        """Wykres ważności cech europejskich"""
        importance_df = self.get_feature_importance()
        if importance_df is None:
            print("❌ Ważność cech niedostępna dla tego modelu")
            return
        ...
        for feature in top_features['feature']:
            if 'polish' in feature.lower() or 'poland' in feature.lower():
                colors.append('red')
            elif any(x in feature.lower() for x in ['airline', 'carrier']):
                colors.append('blue')
            elif any(x in feature.lower() for x in ['region', 'country']):
                colors.append('green')
            elif any(x in feature.lower() for x in ['time', 'hour', 'day', 'month']):
        ...
        plt.title(f'Top {top_n} Najważniejszych Cech - Model Europejski\n🇵🇱 Polskie | ✈️ Przewoźnicy | 🌍 Geografia | ⏰ Czas')
        plt.gca().invert_yaxis()
        
        # Add legend
        legend_elements = [
            plt.Rectangle((0,0),1,1, facecolor='red', label='🇵🇱 Polskie'),
            plt.Rectangle((0,0),1,1, facecolor='blue', label='✈️ Przewoźnicy'),
            plt.Rectangle((0,0),1,1, facecolor='green', label='🌍 Geografia'),
            plt.Rectangle((0,0),1,1, facecolor='orange', label='⏰ Czas'),
            plt.Rectangle((0,0),1,1, facecolor='gray', label='🔧 Inne')
        ]
...
class EuropeanDelayRegressor:
    """
    🇪🇺 Regressor opóźnień - wersja europejska
    Przewiduje liczbę minut opóźnienia dla lotów europejskich
    """
    
    def __init__(self, model_type: str = 'xgboost'):
        """Inicjalizacja europejskiego regressora"""
        self.model_type = model_type
        self.model = None
        self.scaler = StandardScaler()
        self.feature_names = []
        self.classifier = EuropeanDelayClassifier(model_type)  # Użyj tego samego feature engineering
        ...
        if model_type == 'linear':
            ...
            self.model = xgb.XGBRegressor(
                random_state=42, max_depth=6, learning_rate=0.1, 
    ...
    def prepare_features(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        print("🇪🇺 EUROPEAN FEATURE ENGINEERING - REGRESJA")
        print("="*60)
        ...
        # Użyj tego samego feature engineering co klasyfikator
        X, _ = self.classifier.prepare_european_features(delayed_flights, prediction_mode=True)
        y = delayed_flights['delay_minutes']
        
        self.feature_names = self.classifier.feature_names
        
        print(f"✅ European regressor features: {X.shape[1]} cech, {len(y)} próbek")
        return X, y
    
    def train(self, X: pd.DataFrame, y: pd.Series, test_size: float = 0.2):
        """Trenowanie europejskiego modelu regresji"""
        print(f"🇪🇺 TRENOWANIE EUROPEJSKIEGO REGRESSORA: {self.model_type.upper()}")
        print("="*60)
        ...
        print(f"📊 Podział danych:")
        print(f"   • Train: {X_train.shape[0]:,} próbek (średnie opóźnienie: {y_train.mean():.1f} min)")
        print(f"   • Test:  {X_test.shape[0]:,} próbek (średnie opóźnienie: {y_test.mean():.1f} min)")
...
def save_european_models(classifier: EuropeanDelayClassifier, regressor: EuropeanDelayRegressor, 
                        filename_base: str = 'european_best_model'):
    """Zapisuje europejskie modele do plików"""
    ...
    print(f"✅ Europejskie modele zapisane:")
...
def load_european_models(classifier_path: str, regressor_path: str):
    """Ładuje europejskie modele z plików"""
    try:
        classifier = joblib.load(classifier_path)
        regressor = joblib.load(regressor_path)
        
        print(f"✅ Europejskie modele załadowane:")
        print(f"   🎯 Klasyfikator: {classifier_path}")
        print(f"   📈 Regressor: {regressor_path}")
...
def predict_european_delay(flight_details: Dict[str, Any], classifier: EuropeanDelayClassifier, 
                          regressor: EuropeanDelayRegressor) -> Dict[str, Any]:
    """
    Przewidywanie opóźnienia dla lotu europejskiego
    
    Args:
        flight_details (dict): Szczegóły lotu
        classifier: Wytrenowany klasyfikator
        regressor: Wytrenowany regressor
        
    Returns:
        dict: Wyniki przewidywania
    """
    ...
    try:
        ...
        # Feature engineering
        X_clf, _ = classifier.prepare_european_features(df_flight, prediction_mode=True)
        
        # Przewidywanie klasyfikacji (czy opóźniony?)
